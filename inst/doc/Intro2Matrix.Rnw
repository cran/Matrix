...
reasons, we use ``zero-based'' indexing in the \code{Matrix} package, i.e.,
...
the \class{TsparseMatrix} class has several ``actual'' subclasses,
the most typical (numeric, general) is \class{dgTMatrix}:
<<Tsparse-class>>=
getClass("TsparseMatrix")  # (i,j, Dim) slot are common
                          ## see the sub classes
getClass("dgTMatrix")
@
...
\subsubsection{Compressed representations: \class{CsparseMatrix} }
% (and \class{RsparseMatrix})} -- but don't advertize them
