...
SEXP CHMfactor_to_sparse(SEXP x)
{
    ...
    /* cholmod_l_factor_to_sparse changes its first argument. Make a copy */
    Lcp = cholmod_l_copy_factor(L, &c);
    if (!(Lcp->is_ll))
	if (!cholmod_l_change_factor(Lcp->xtype, 1, 0, 1, 1, Lcp, &c))
	    error(_("cholmod_l_change_factor failed with status %d"), c.status);
    Lm = cholmod_l_factor_to_sparse(Lcp, &c); cholmod_l_free_factor(&Lcp, &c);
...
SEXP CHMfactor_spsolve(SEXP a, SEXP b, SEXP system)
{
    ...
    if (!(sys--))		/* -- align with CHOLMOD defs */
	error(_("system argument is not valid"));
    return chm_sparse_to_SEXP(cholmod_l_spsolve(sys, L, B, &c),
