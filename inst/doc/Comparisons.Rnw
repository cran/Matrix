...
mm = as(mm, "matrix")
dim(mm)
naive.sol = solve(t(mm) %*% mm) %*% t(mm) %*% y
system.time(solve(t(mm) %*% mm) %*% t(mm) %*% y)
system.time(solve(t(mm) %*% mm) %*% t(mm) %*% y)
system.time(solve(t(mm) %*% mm) %*% t(mm) %*% y)
@ 
(Here and in what follows we will repeat timings four times to obtain
timings that are close to the steady-state values.)
...
<<crossKoenNg>>=
cpod.sol = solve(crossprod(mm), crossprod(mm,y))
