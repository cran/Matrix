...
int LaGenMatComplex::debug_ = 0;     // turn off global deubg flag initially.
                                // use A.debug(1) to turn on/off,
                                // and A.debug() to check current status.

int* LaGenMatComplex::info_= new int;        // turn off info print flag.

LaGenMatComplex::~LaGenMatComplex()
{}

 LaGenMatComplex::LaGenMatComplex() : v(0)
{
  ...
  LaGenMatComplex::LaGenMatComplex(int m, int n) :v(m*n)
{
    ii[0](0,m-1);
    ii[1](0,n-1);
    dim[0] = m;
  ...
  LaGenMatComplex::LaGenMatComplex(COMPLEX *d, int m, int n) :v(d, m*n)
{
    if (debug())
    {
        cout << ">>> LaGenMatComplex::LaGenMatComplex(COMPLEX *d, int m, int n) :v(d, m*n)\n";
    }
...
if (debug())
{
        cout << ">>> LaGenMatComplex& LaGenMatComplex::operator=(COMPLEX s)\n";
        cout << "       s = " << s << endl;
}
...
LaGenMatComplex&  LaGenMatComplex::resize(int m, int n)
{
    if (debug())
    {
        cout << ">>> resize("<< m << "," << n << ")\n";
    ...
    LaGenMatComplex tmp1(0,0);
    ref(tmp1);

    // now, reference an MxN matrix
    LaGenMatComplex tmp(m,n);
    ref(tmp);
...
if (debug())
{
 ...
 LaGenMatComplex&  LaGenMatComplex::resize(const LaGenMatComplex &A)
{
    if (debug())
    {
        cout << ">>> resize(const LaGenMatComplex &A)" << endl;
        cout << "      A: " << A.info() << endl;
    }
...
LaGenMatComplex::LaGenMatComplex(const LaGenMatComplex& X) : v(0)
{
    if (X.debug())
    {
        cout << ">>> LaGenMatComplex::LaGenMatComplex(const LaGenMatComplex& X)\n";
        cout << "X: " << X.info() << endl;
...
LaGenMatComplex& LaGenMatComplex::copy(const LaGenMatComplex& X) 
{
    if (debug())
    {
        cout << ">>> LaGenMatComplex& LaGenMatComplex::copy\
(const LaGenMatComplex& X)\n";
        cout << " X: " << X.info() << endl;
    }
...
if (debug())
{
    cout << ">>> LaGenMatComplex::operator(const LaIndex& const LaIndex&)\n";
}
    LaIndex I, J;
...
ostream& operator<<(ostream& s, const LaGenMatComplex& G)
{
