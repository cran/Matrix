...
mm = as(mm, "matrix")
dim(mm)
sysgc.time(naive.sol <- solve(t(mm) %*% mm) %*% t(mm) %*% y)
@ 
...
<<crossKoenNg>>=
sysgc.time(cpod.sol <- solve(crossprod(mm), crossprod(mm,y)))
...
mm = as(mm, "geMatrix")
class(crossprod(mm))
sysgc.time(Mat.sol <- solve(crossprod(mm), crossprod(mm, y)))
...
data(mm, package = "Matrix")
class(mm)
sysgc.time(sparse.sol <- solve(crossprod(mm), crossprod(mm, y)))
