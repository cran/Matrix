...
\title{Introduction to the Matrix package}
...
\begin{description}
\item[geMatrix] Real matrices in general storage mode
\item[syMatrix] Symmetric real matrices in non-packed storage
\item[trMatrix] Triangular real matrices in non-packaged storage
\item[poMatrix] Positive semi-definite symmetric real matrices in
  non-package storage
\end{description}
Methods for these classes include coersion between these classes, when
appropriate, and coersion to the \code{matrix} class; methods for
...
preserved as a (list) element in the \code{factorization} slot of the
...
\begin{description}
\item[tripletMatrix] general, real, sparse matrices in (a possibly
  redundant) triplet form.  This can be a convenient form in which to
  construct sparse matrices.
\item[cscMatrix] general, real, sparse matrices in the (sorted) compressed
  sparse column format.
\item[sscMatrix] symmetric, real, sparse matrices in the (sorted)
...
\item[tscMatrix] triangular, real, sparse matrices in the (sorted)
